...
def pdf_gen_en(string):
    GRID_ROW_NUM=25
    packet = io.BytesIO()
    c = canvas.Canvas(packet,pagesize=A4)
    filter_non_en=re.compile('[^\w]')
    string=filter_non_en.sub(r' ',string)
    #words=nltk.word_tokenize(string)
    words=re.split(r'[ ,|;"]+', string)

    margin, grid_size=30,30 #define page margin
    page_height=A4[1]-margin
    page_width=A4[0]-margin
    page_total=len(words)//GRID_ROW_NUM+1
    print ('Total Page: ', page_total)
    for _ in range(page_total):
        for i in range(GRID_ROW_NUM):
            try:
            ...
            c.setDash(2,1)
            c.setLineWidth(0.5)
            c.line(margin,page_height-0.7*margin-i*(grid_size)+8,page_width,page_height-0.7*margin-i*(grid_size)+8 )
            c.line(margin,page_height-0.7*margin-i*(grid_size)+1,page_width,page_height-0.7*margin-i*(grid_size)+1 )
            c.setDash(1,0)            
            c.line(margin,page_height-0.7*margin-i*(grid_size)+15,page_width,page_height-0.7*margin-i*(grid_size)+15 )           
            c.line(margin,page_height-0.7*margin-i*(grid_size)-5,page_width,page_height-0.7*margin-i*(grid_size)-5 )

        c.showPage()

    c.save()
    new_pdf_file_name=os.path.join(os.path.dirname(__file__), 'En.'+str(datetime.timestamp(datetime.now()))+'.pdf')
    pdf=open(new_pdf_file_name,'wb')
    pdf.write(packet.getvalue())     # Finally output new pdf
...
b_ok=Button(frame3,command=grid_lines,text='OK',width=15,fg='green', font='Tahoma 10 bold')
